...
class Program
{
    ...
    public static async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
    {
        ...
        if (update.Type == UpdateType.Message)
        {
            ...
            {
                await botClient.SendTextMessageAsync(message.Chat.Id, "<b>хуйло‼ СЛАВА УКРАЇНІ</b>",ParseMode.Html);
                return;
            }
            ...
            else if (message.Text.ToLower().StartsWith("/search"))
            {
                ...
                {
                    string apiUrl = $"https://localhost:7297/api/Movies/{searchQuery}"; // URL вашої API
            ...
            else if (message.Text.ToLower()=="/my_reviews")
            {
                ...
                int index = 0;
                string uri=$"https://localhost:7297/api/Review/GetMyReviewByIndex?tgUsername={message.From}&index={index}";
            ...
            if (message.ReplyToMessage != null && message.ReplyToMessage.From.Id == botClient.BotId)
            {
                if (update.Message.ReplyToMessage.Text.StartsWith("Write a review for "))
                {
                    ...
                    var request = new HttpRequestMessage
                    {
                        ...
                        RequestUri = new Uri($"https://localhost:7297/api/Review/PostReview?tgUsername={message.From}&movieId={title}&marking=-1&text={message.Text}"),
                    };
                    using (var response = await client.SendAsync(request))
                    {
                        response.EnsureSuccessStatusCode();
                ...
                else if(update.Message.ReplyToMessage.Text.StartsWith("Rewrite a review for "))
                {
                    ...
                    var uri = $"https://localhost:7297/api/Review/DeleteReview?tgUsername={update.Message.From}&movieId={title}";
        ...
        else if (update.Type == UpdateType.CallbackQuery)
        {
            ...
            switch (update.CallbackQuery.Data)
            {
                case "Reviews":
                    ...
                    var uri = $"https://localhost:7297/api/Review/reviews?Title={title}&index={ind}";
                    var client = new HttpClient();
                    var request = new HttpRequestMessage
                    {
                        Method = HttpMethod.Get,
                ...
                case "❌":
                    ...
                    uri = $"https://localhost:7297/api/Review/DeleteReview?tgUsername={update.CallbackQuery.From}&movieId={title}";
                    HttpClient httpClient = new HttpClient();
                    HttpResponseMessage response = await httpClient.DeleteAsync(uri);
                    ind = 0;
                    uri = $"https://localhost:7297/api/Review/GetMyReviewByIndex?tgUsername={update.CallbackQuery.From}&index={ind}";
            ...
            {
                var client = new HttpClient();
                var uri = $"https://localhost:7297/api/Review/GetMyReviewByIndex?tgUsername={update.CallbackQuery.From}&index={index}";
                    ...
                    var buttons = new InlineKeyboardMarkup(new[]
{
            ...
            else if (int.TryParse(update.CallbackQuery.Data.Split(",").Last(), out index))
            {
                ...
                Console.WriteLine(title);
                var client = new HttpClient();
                var uri = $"https://localhost:7297/api/Review/reviews?Title={title}&index={index}";
